...
Item* Player::StoreItem(ItemPosCountVec const& dest, Item* pItem, bool update)
{
    ...
    if (pItem->GetProto()->Quality > 2 && pItem->GetProto()->Flags != 2048 && (pItem->GetProto()->Class == ITEM_CLASS_WEAPON || pItem->GetProto()->Class == ITEM_CLASS_ARMOR))
    {
        if (pItem->GetOwner())
            pItem->GetOwner()->CreateWowarmoryFeed(2, entry, pItem->GetGUIDLow(), pItem->GetProto()->Quality);
    }
    /* World of Warcraft Armory */
